...
export function ArtifactListItem({ artifact, selected, onSelect }: ArtifactListItemProps) {
  ...
  const button = (
    <button
      ...
      aria-describedby={artifact.subtitle ? tooltipId : undefined}
    ...
    <div className="group relative text-sm">
      {artifact.subtitle ? (
        <Tooltip.Root disableHoverablePopup>
          ...
          <Tooltip.Portal>
            <Tooltip.Positioner side="bottom" align="start" sideOffset={8} className="z-50">
              <Tooltip.Popup
                id={tooltipId}
                role="tooltip"
                className="w-64 border border-gray-200 bg-white px-2.5 py-2 text-xs leading-snug text-gray-700 shadow-sm dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"
              >
                {artifact.subtitle}
              </Tooltip.Popup>
            </Tooltip.Positioner>

...
Write for a useful life and technical blog. The tone should be down to earth and clear: explain like I'm an intern, not explain like I'm five. Preserve my voice where the conversation gives you enough signal. Every line should earn its keep. Hooks and hook-like lines are allowed when they genuinely help the reader stay oriented, but use them sparingly. Avoid hype, filler, buzzwords, generic lessons, and anything that sounds like a content-marketing recap.
...
- Concision, style, and whether every line earns its keep.
...
Keep the summary specific and useful, not promotional. Use the article H1 or title unless it is clearly a placeholder. Prefer a small, high-signal tag and keyword set over an exhaustive list. Entities should be concrete people, products, technologies, organizations, projects, concepts, or documents that matter to the article. Semantic triples should capture the article's most important knowledge-graph-worthy claims in plain language. Do not include placeholder entities or triples; use [] when none are warranted. Quote or escape YAML strings as needed.

...
test('artifact list item keeps metadata inside the selection button and standalone link separate', () => {
  ...
  assert.match(buttonHtml, /aria-describedby="/);
  ...
  // Tooltip popup is closed by default; nothing with role="tooltip" is rendered.
  assert.doesNotMatch(html, /role="tooltip"/);
});

test('artifact list item without a subtitle renders a plain button with no tooltip wiring', () => {
  const html = renderToStaticMarkup(
    React.createElement(
      'ul',
      null,
  ...
  assert.match(html, /aria-label="Select Plain"/);
